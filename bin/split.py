...
if __name__ == '__main__':
    ...
    with pd.ExcelWriter(path_train) as writer:
        X_train.to_excel(writer, sheet_name='input', index=True, header=True)
        y_train.to_excel(writer, sheet_name='output', index=True, header=True)

    with pd.ExcelWriter(path_test) as writer:
        X_test.to_excel(writer, sheet_name='input', index=True, header=True)
        y_test.to_excel(writer, sheet_name='output', index=True, header=True)
